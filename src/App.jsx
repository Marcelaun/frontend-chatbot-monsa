...
const CONFIG = {
  logoUrl:
    "https://ui-avatars.com/api/?name=OSC+MONSA&background=3c6bee&color=fff",
  nomeOng: "OSC MONSA",
  siglaOng: "OM",
...
const FLOW_SEQUENCE = [
  {
    id: "nome_responsavel",
    question: `Olá! Sou o assistente virtual do Centro Cultural ${CONFIG.nomeOng}. Primeiramente, com quem eu estou falando?`,
    type: "form",
    fields: [
      {
        id: "nome",
  ...
  },
  {
    id: "nome_inscrito",
    question: (data) =>
      `Prazer em te conhecer, ${data?.nome_responsavel || ""}! Qual é o nome completo da pessoa que será inscrita?`,
    type: "form",
    fields: [
      { id: "nome", label: "Nome do Inscrito", placeholder: "Nome completo" },
    ],
  },
  {
    id: "data_nasc",
  ...
  {
    id: "endereco",
    question: "Certo. Qual é o endereço completo? Rua, Número e Bairro.",
    type: "form",
    fields: [
      {
        id: "endereco",
  ...
  {
    id: "rg_cpf",
    question: "Agora preciso dos números de RG e CPF do inscrito.",
    type: "form",
    fields: [
      {
        id: "rg",
  ...
  },
  {
    id: "filiacao",
    question:
      "Qual o nome da Filiação (Pai, Mãe ou Responsável) e o CPF do responsável?",
    type: "form",
    fields: [
      {
        id: "nome",
  ...
  },
  {
    id: "familiares",
    question:
      "Quais pessoas moram na mesma casa? Pode ditar os nomes e parentescos.",
    type: "form",
    fields: [
      {
        id: "familiares",
        label: "Moradores da residência",
        type: "textarea",
        placeholder: "Liste os moradores e parentesco",
      },
    ],
  },
  {
    id: "renda_auxilio",
    question:
      "Qual a renda mensal da família e recebem algum auxílio do governo?",
    type: "form",
    fields: [
      {
        id: "renda",
      ...
      {
        id: "auxilio",
        label: "Auxílio do Governo",
        placeholder: 'Ex: Bolsa Família ou "Nenhum"',
      },
    ],
  },
  {
    id: "saude",
    question:
      "Alguém na família toma algum remédio controlado ou precisa de atenção à saúde?",
    type: "form",
    fields: [
      {
        id: "saude",
        ...
        type: "textarea",
        placeholder: 'Descreva ou digite "Não"',
      },
    ],
  },
  {
    id: "expectativa",
    question: `Qual a sua expectativa no trabalho da ${CONFIG.nomeOng} com seu filho ou filha?`,
    type: "form",
    fields: [
      {
        id: "expectativa",
        ...
        type: "textarea",
        placeholder: "Digite sua expectativa",
      },
    ],
  },
  {
    ...
    options: [
      "Sim, temos",
      "Não temos, mas tenho interesse",
      "Não temos e sem interesse",
    ],
  },
  {
    id: "cursos",
    question: "Quais cursos de interesse você tem vontade de fazer?",
    type: "form",
    fields: [
      {
        id: "cursos",
        label: "Cursos de interesse",
        placeholder: "Ex: Corte e Costura, Informática...",
      },
    ],
  },
  {
    id: "foto_doc",
    question:
      "Quase lá! Por favor, tire uma foto do documento, RG ou CPF, ou escolha na galeria.",
    type: "file",
    accept: "image/*",
  },
  {
    id: "foto_crianca",
    question: "Ótimo! Agora, por favor, envie a foto da criança inscrita.",
    type: "file",
    accept: "image/*",
  },
];
...
export default function App() {
  ...
  const speakText = (text) => {
    ...
    const cleanText = text
      .replace(
        /[\u{1F600}-\u{1F6FF}\u{1F300}-\u{1F5FF}\u{2700}-\u{27BF}]/gu,
        "",
      )
      .replace(/\*/g, "");
...
Exemplo: { "valid": true, "extracted_value": "valor extraído", "error_message": null }`;
  ...
  const handleNextStep = async (
    ...
    if (!skipValidation) {
      const questionText =
        typeof stepData.question === "function"
          ? stepData.question(formData)
          : stepData.question;
      const expectedType =
        stepData.type === "form"
          ? `Form: ${stepData.fields.map((f) => f.label).join(", ")}`
          : stepData.type;
      ...
      if (!validationResult.valid) {
        setIsTyping(false);
        addBotMessage(
          `Hmm, acho que precisamos corrigir isso 🤔\n\n${validationResult.error_message}`,
        );
        return;
      }
      finalValueToSave =
        stepData.type === "form" && stepData.fields.length > 1
          ? valueToSave
  ...
  const generateXML = (data) => {
    ...
    const familiaresLinhas = familiaresTexto
      .split("\n")
      .filter((line) => line.trim() !== "");
    const familiaresXml = familiaresLinhas
      .map(
        (linha) =>
...
<cadastro>
  ...
  const finishCadastro = async (finalData) => {
    ...
    try {
      ...
      const getVal = (field, key) =>
        typeof field === "object" && field !== null ? field[key] : field;
      const nomeCriancaStr =
        getVal(finalData.nome_inscrito, "nome") || "Inscrito_Novo";
      ...
      const WEBHOOK_URL =
        "https://autobackend.duckdns.org/webhook-test/monsa-cadastro";
      ...
      <div className="min-h-[100dvh] w-full bg-gradient-to-br from-blue-50 via-gray-100 to-blue-200 flex items-center justify-center md:p-6 font-sans">
        ...
        <div className="w-full h-[100dvh] md:w-[420px] md:h-[85vh] md:min-h-[600px] md:max-h-[800px] bg-blue-700 md:rounded-[2rem] md:shadow-2xl overflow-hidden flex flex-col items-center justify-center text-white p-6 text-center md:border md:border-blue-800">
          ...
          <p className="text-blue-100">
            Enviando seus documentos de forma segura para a ONG.
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-[100dvh] w-full bg-gradient-to-br from-blue-50 via-gray-100 to-blue-200 flex items-center justify-center md:p-6 font-sans">
        ...
        <div className="w-full h-[100dvh] md:w-[420px] md:h-[85vh] md:min-h-[600px] md:max-h-[800px] bg-gray-50 md:rounded-[2rem] md:shadow-2xl overflow-hidden flex flex-col items-center justify-center p-6 text-center animate-fade-in md:border md:border-gray-200">
          ...
          <p className="text-gray-600 mb-8 max-w-sm">
            Seu pré-cadastro na {CONFIG.nomeOng} foi concluído. Lembre-se de
      ...
      <div className="w-full h-[100dvh] md:w-[420px] md:h-[85vh] md:min-h-[600px] md:max-h-[800px] bg-gray-50 md:rounded-[2rem] md:shadow-2xl overflow-hidden flex flex-col relative md:border md:border-gray-200">
        <header className="bg-blue-700 text-white p-4 shadow-md flex items-center justify-between shrink-0 z-10 relative">
          ...
          <button
            ...
            title={
              isVoiceBotEnabled ? "Desativar voz do bot" : "Ativar voz do bot"
            }
          >
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4 relative">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`flex ${msg.sender === "bot" ? "justify-start" : "justify-end"} animate-fade-in`}
            >
              {msg.sender === "bot" && (
              ...
              <div
                className={`max-w-[80%] rounded-2xl p-3 text-sm shadow-sm ${msg.sender === "bot" ? "bg-white text-gray-800 rounded-bl-none border border-gray-100 whitespace-pre-line" : "bg-blue-600 text-white rounded-br-none"}`}
              >
                {msg.text}
              </div>
            </div>
...
function InputResolver({
  ...
  const toggleSpeechToText = (fieldId, currentMask) => {
    ...
    if (!SpeechRecognition) {
      alert(
        "Seu navegador não suporta digitação por voz. Tente usar o Google Chrome.",
      );
      return;
    }
  ...
  const handleFormSubmit = (e) => {
    ...
    if (
      !stepData.fields.every(
        (f) => localValues[f.id] && localValues[f.id].trim() !== "",
      )
    )
      return;
    let answerText =
      stepData.fields.length === 1
        ? localValues[stepData.fields[0].id]
        : stepData.fields
            .map((f) => `${f.label}: ${localValues[f.id]}`)
            .join("\n");
    let valueToSave =
      stepData.fields.length === 1
        ? localValues[stepData.fields[0].id]
    ...
    return (
      <div className="flex flex-col items-center">
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">
          Tire uma foto nítida em local iluminado.
        </p>
      </div>
    );
  ...
  if (stepData.type === "form") {
    const isSingleField =
      stepData.fields.length === 1 && stepData.fields[0].type !== "textarea";
    const isAllFilled = stepData.fields.every(
      (f) => localValues[f.id] && localValues[f.id].trim() !== "",
    );
    ...
    return (
      <form
        ...
        className={`flex ${isSingleField ? "flex-row items-end gap-2" : "flex-col gap-3"} w-full`}
      >
        <div
          className={`flex flex-col gap-3 ${isSingleField ? "flex-1" : "w-full"}`}
        >
          {stepData.fields.map((field) => (
            <div key={field.id} className="flex flex-col relative">
              {field.label && !isSingleField && (
                <span className="text-xs font-semibold text-blue-800 ml-1 mb-1">
                  {field.label}
                </span>
              )}
              ...
              <div className="relative flex items-center w-full">
                {field.type === "textarea" ? (
                  <textarea
                    ...
                    onChange={(e) =>
                      handleChange(field.id, e.target.value, field.mask)
                    }
